...
const DeactiveSellers = () => {
  ...
  const { deactivesellers, totaldeactivesellers } = useSelector(
    (state) => state.seller
  );
  ...
  useEffect(() => {
    ...
    dispatch(get_deactive_sellers(object));
  ...
  return (
    <div className="px-2 lg:px-7 pt-5">
      <div className="w-full p-4  bg-[#283046] rounded-md">
              ...
              {deactivesellers.map((d, i) => (
                <tr key={i}>
                  ...
                  <td
                    scope="row"
                    className="py-3 px-4 font-medium whitespace-nowrap">
                    <span>{d.shopInfo && d.shopInfo.busnessname}</span>
                  </td>
                  <td
                    scope="row"
                    className="py-3 px-4 font-medium whitespace-nowrap">
                  ...
                  <td
                    ...
                    {d.shopInfo && (
                      <div>
                        {d.shopInfo.busnessname && (
                          <span>{d.shopInfo.busnessname}</span>
                        )}
                        {d.shopInfo.addressione && (
                          <span>{d.shopInfo.addressione}</span>
                        )}
                        {d.shopInfo.addressitwo && (
        ...
        {totaldeactivesellers > perPage && (
          <div className="w-full flex justify-end mt-4 bottom-4 right-4">
            <Pagination
              pageNumber={currentPage}
              setPageNumber={setCurrentPage}
              totalItem={totaldeactivesellers}
              parPage={perPage}
              showItem={4}
            />
          </div>
